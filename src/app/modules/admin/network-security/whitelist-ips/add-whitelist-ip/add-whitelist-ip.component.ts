...
@Component({
    selector: 'app-add-whitelist-ip',
    templateUrl: './add-whitelist-ip.component.html',
    styleUrls: ['./add-whitelist-ip.component.scss']
})
export class AddWhitelistIpComponent implements OnInit {
    ...
    addWhitelistIPForm!: FormGroup;
    
    constructor(public dialogRef: MatDialogRef<AddWhitelistIpComponent>, @Inject(MAT_DIALOG_DATA) public data: any, 
    ...
    onSubmit() 
    {
        if (this.addWhitelistIPForm.invalid) {
            this.showAlert = true;
        ...
        const formValue = this.addWhitelistIPForm.value;
        ...
        // Call API
        this.nicService.addWhitelistIP(formData).subscribe(
            (response) => {
