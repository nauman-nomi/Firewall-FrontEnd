...
def run_block_script():
    try:
        result = subprocess.run(
            ["/usr/local/bin/doas", "/bin/sh", BLOCK_SCRIPT_PATH],
...
elif "/run" in path_info:
    ok, output = run_block_script()
    if ok:
        response = {"success": True, "message": f"Script executed successfully: {output}"}
    else:
        response = {"success": False, "message": f"Failed to execute script: {output}"}
